var locations = {
    ...
    6: {
        ...
        scripts: ['customobjselect.js', 'customobjshadow.js', 'EffectComposer.js', 'ShaderPass.js', 'houses/HorizontalBlurShader.js', 'houses/VerticalBlurShader.js', 'houses/CopyShader.js', 'wintertrees/BloomPass.js', 'wintertrees/BleachBypassShader.js',
            'wintertrees/ConvolutionShader.js'
        ],
    ...
    12: {
        ...
        onInit: function() {
            ...
            this.listener = new BABYLON.AudioListener();
            chat3d.world.camera.add(this.listener);


            /*this.sound = new BABYLON.PositionalAudio(this.listener);
            this.sound.position.x = 400;
            this.sound.position.y = 120;
            this.sound.setRefDistance(2000);
            //chat3d.world.obj.push(this.sound);
            chat3d.world.scene.add(this.sound);

            var audioLoader = new BABYLON.AudioLoader();
            audioLoader.load('data/sounds/street1_right.mp3', function(buffer) {
                this.sound.setBuffer(buffer);
                this.sound.setLoop(true);
                this.sound.setVolume(10);
                this.sound.ready = true;
            ...
            this.sound2 = new BABYLON.PositionalAudio(this.listener);
            this.sound2.position.x = -400;
            this.sound2.position.y = 120;
            this.sound2.setRefDistance(2000);
            //chat3d.world.obj.push(this.sound);
            chat3d.world.scene.add(this.sound2);

            var audioLoader2 = new BABYLON.AudioLoader();
            audioLoader2.load('data/sounds/street1_left.mp3', function(buffer) {
                this.sound2.setBuffer(buffer);
                this.sound2.setLoop(true);
                //this.sound2.setVolume(10);
                this.sound2.ready = true;
                //console.log(this.sound)
            }.bind(this));*/
            ...
            this.sgroup[0] = document.createElement('Audio');
            this.sgroup[0].rowVolume = 1;
            this.sgroup[0].oncanplay = function() {
                that.sgroup[0].ready = true;
                that.sgroup[0].loop = true
            };
            this.sgroup[0].src = 'data/sounds/stereo_street.mp3';

            this.sgroup[1] = document.createElement('Audio');
            this.sgroup[1].rowVolume = 0;
            this.sgroup[1].oncanplay = function() {
                that.sgroup[1].ready = true;
                that.sgroup[1].loop = true
                    // that.sgroup[1].currentTime = parseInt(that.sgroup[1].duration / 2);
                that.sgroup[1].volume = 0.0;
            };
            this.sgroup[1].src = 'data/sounds/stereo_street.mp3';
            ...
            let playBut = $('<div/>')
                ...
                .click(function() {
                    ...
                    if (that.sgroup[1].ready && that.sgroup[0].ready) {
                        setTimeout(function() {
                            console.log('play');
                            that.sgroup[1].play();
                        }, that.sgroup[1].duration * 0.8 * 1000);
                        that.sgroup[0].play();
                    }
                });
            $('body').append(playBut);

            //console.log(chat3d.world.googlemap);


            /*geocoder.geocode({ 'latLng': new google.maps.LatLng(chat3d.world.googlemap.lat, chat3d.world.googlemap.lng) },
        ...
        onLoop: function(delta) {
            let cTime1 = this.sgroup[0].currentTime / this.sgroup[0].duration;
            let cTime2 = this.sgroup[1].currentTime / this.sgroup[1].duration;
            //console.log(cTime1, cTime2);
            if (cTime1 > 0.9) {
                if (this.sgroup[1].rowVolume == 1) {
                    let vol1 = this.sgroup[0].rowVolume;
                    vol1 -= delta * 1.5;
                    vol1 = Math.max(0, vol1);
                    this.sgroup[0].rowVolume = vol1;
                }

                let vol2 = this.sgroup[1].rowVolume;
                vol2 += delta * 1.5;
                vol2 = Math.min(1, vol2);
                this.sgroup[1].rowVolume = vol2;
            ...
            if (cTime2 > 0.9) {
                if (this.sgroup[0].rowVolume == 1) {
                    let vol1 = this.sgroup[1].rowVolume;
                    vol1 -= delta * 1.5;
                    vol1 = Math.max(0, vol1);
                    this.sgroup[1].rowVolume = vol1;
                }

                let vol2 = this.sgroup[0].rowVolume;
                vol2 += delta * 1.5;
                vol2 = Math.min(1, vol2);
                this.sgroup[0].rowVolume = vol2;

                //console.log('switch 2');
            }
            if (chat3d.video_sound) {
                this.soundVol += delta;
                this.soundVol = Math.min(1, this.soundVol);
            } else {
                this.soundVol -= delta;
                this.soundVol = Math.max(0, this.soundVol);
            }

            this.sgroup[0].volume = this.sgroup[0].rowVolume * this.soundVol;
            this.sgroup[1].volume = this.sgroup[1].rowVolume * this.soundVol;
        },

        onRender: function(delta) {

            //this.sound.setVolume(5 * chat3d.video_sound);
            //this.sound2.setVolume(5 * chat3d.video_sound);
        ...
        onClean: function() {
            ...
            //chat3d.world.scene.remove(this.sound);
            //chat3d.world.scene.remove(this.sound2);
            //this.sound.pause();
            //this.sound2.stop();
            this.sgroup[0].pause();
            this.sgroup[1].pause();
    ...
    23: {
        ...
        obj: [
            { "id": "est_loc_01" }, { "id": "est_loc_01_collider" }, { "id": "pine_tree_004" }, { "id": "pine_tree_006" }, { "id": "tree_13a_001" }, { "id": "tree_13c_001" }, { "id": "tree_13a_002" }, { "id": "est_loc_01_bgr_trees" }
            /*{ id: "est_loc_03_transparent_objects" },  { "id": "est_loc_01" }, { "id": "est_loc_01" }, { "id": "est_loc_01_collider" }, { "id": "est_loc_01_collider" }, { "id": "pine_tree_001" }, { "id": "pine_tree_001" }, { "id": "pine_tree_003" }, { "id": "pine_tree_003" }, { "id": "pine_tree_004" }, { "id": "pine_tree_004" }, { "id": "pine_tree_005" }, { "id": "pine_tree_005" }, { "id": "pine_tree_006" }, { "id": "pine_tree_006" }, { "id": "tree_13b_001" }, { "id": "tree_13b_001" }, { "id": "tree_13a_001" }, { "id": "tree_13a_001" }, { "id": "tree_13c_001" }, { "id": "tree_13c_001" }, { "id": "tree_13a_002" }, { "id": "tree_13a_002" }, { "id": "tree_13c_002" }, { "id": "tree_13c_002" }, { "id": "tree_13b_002" }, { "id": "tree_13b_002" }*/
            /*{ "id": "est_loc_01" }, { "id": "est_loc_01" }, { "id": "mc_fir_1_001" }, { "id": "mc_fir_1_001" }, { "id": "mc_pine_3_001" }, { "id": "mc_pine_3_001" }, { "id": "mc_pine_2_002" }, { "id": "mc_pine_2_002" }, { "id": "mc_fir_2_005" }, { "id": "mc_fir_2_005" }, { "id": "mc_fir_2_004" }, { "id": "mc_fir_2_004" }, { "id": "mc_fir_4_001" }, { "id": "mc_fir_4_001" }, { "id": "mc_fir_2_001" }, { "id": "mc_fir_2_001" }, { "id": "mc_fir_2_002" }, { "id": "mc_fir_2_002" }, { "id": "mc_pine_3_003" }, { "id": "mc_pine_3_003" }, { "id": "mc_pine_2_005" }, { "id": "mc_pine_2_005" }, { "id": "mc_fir_2_006" }, { "id": "mc_fir_2_006" }, { "id": "mc_pine_1_004" }, { "id": "mc_pine_1_004" }, { "id": "mc_pine_3_008" }, { "id": "mc_pine_3_008" }, { "id": "mc_pine_3_005" }, { "id": "mc_pine_3_005" }, { "id": "mc_fir_3_005" }, { "id": "mc_fir_3_005" }, { "id": "mc_fir_4_003" }, { "id": "mc_fir_4_003" }, { "id": "mc_fir_3_004" }, { "id": "mc_fir_3_004" }, { "id": "mc_fir_3_003" }, { "id": "mc_fir_3_003" }, { "id": "mc_fir_3_002" }, { "id": "mc_fir_3_002" }, { "id": "mc_pine_2_006" }, { "id": "mc_pine_2_006" }, { "id": "mc_pine_1_003" }, { "id": "mc_pine_1_003" }, { "id": "mc_pine_1_002" }, { "id": "mc_pine_1_002" }, { "id": "mc_pine_3_002" }, { "id": "mc_pine_3_002" }, { "id": "mc_pine_1_001" }, { "id": "mc_pine_1_001" }, { "id": "est_loc_01_collider" }, { "id": "est_loc_01_collider" }*/
            /*{ "id": "est_loc1_ground" }, { "id": "mc_flowers_3" }, { "id": "mc_pine_1_001" }, { "id": "mc_pine_3_002" }, { "id": "mc_pine_1_002" }, { "id": "mc_pine_1_003" }, { "id": "mc_pine_2_006" }, { "id": "mc_fir_4_003" }, { "id": "mc_pine_3_005" }, { "id": "mc_pine_3_008" }, { "id": "mc_pine_1_004" }, { "id": "mc_fir_2_006" }, { "id": "mc_pine_2_005" }, { "id": "mc_pine_3_003" }, { "id": "mc_fir_2_003" }, { "id": "mc_fir_2_002" }, { "id": "mc_fir_2_001" }, { "id": "mc_fir_4_001" }, { "id": "mc_fir_2_004" }, { "id": "mc_pine_2_002" }, { "id": "mc_pine_3_001" }, { "id": "mc_pine_3_007" }, { "id": "mc_fir_1_001" }, { "id": "mc_fir_2_007" }, { "id": "est_loc_01_collider" }, { "id": "mc_fir_3_002" }, { "id": "mc_fir_3_003" }, { "id": "mc_fir_3_004" }, { "id": "mc_fir_3_005" }, { "id": "mc_fir_3_006" }, { "id": "mc_fir_3_001" },*/
        ...
        scripts: ['houses/main.js', 'p2player.js', 'babylonjs/TessellateModifier.js', 'babylonjs/SubdivisionModifier.js', 'houses/EffectComposer.js', 'houses/HorizontalBlurShader.js', 'houses/VerticalBlurShader.js', 'houses/RenderPass.js', 'houses/ShaderPass.js', 'houses/CopyShader.js', 'houses/SimplifyModifier.js'],
    ...
    24: {
        ...
        z: 50,
        textures: ['pokazyvalka.png', 'pokazyvalka2.png', '3d_icon_downstairs.png', '3d_icon_enter.png', '3d_icon_exit.png', '3d_icon_upstairs.png', 'lenghtarrow.png', 'dotlength.png', 'area_room_white.png'],
        scripts: ['houses/main.js', 'p2player.js', 'houses/EffectComposer.js', 'houses/HorizontalBlurShader.js', 'houses/VerticalBlurShader.js', 'houses/RenderPass.js', 'houses/ShaderPass.js', 'houses/CopyShader.js', 'real_mirror.js', 'FXAAShader.js', 'houses/SimplifyModifier.js']
    },
    ...
    25: {
        ...
        z: 250,
        textures: ['pokazyvalka.png', 'pokazyvalka2.png', '3d_icon_downstairs.png', '3d_icon_enter.png', '3d_icon_exit.png', '3d_icon_upstairs.png', 'lenghtarrow.png', 'dotlength.png', 'area_room_white.png'],
        scripts: ['houses/main.js', 'p2player.js', 'houses/EffectComposer.js', 'houses/HorizontalBlurShader.js', 'houses/VerticalBlurShader.js', 'houses/RenderPass.js', 'houses/ShaderPass.js', 'houses/CopyShader.js', 'houses/SimplifyModifier.js']
    },
    ...
    27: {
        ...
        mobileLook: -10,
        obj: [{ "id": "est_loc_02" }, { "id": "mc_plant_116_004_2" }, { "id": "mc_plant_101_004_2" }, { "id": "mc_plant_101_003_2" }, { "id": "mc_plant_101_005_2" }, { "id": "mc_plant_103_002_2" }, { "id": "mc_plant_103_001_2" }, { "id": "mc_plant_101_006_2" }, { "id": "mc_plant_116_005_2" }, { "id": "mc_plant_101_001_2" }, { "id": "mc_plant_101_002_2" }, { "id": "est_loc_02_collider" }, { "id": "mc_fir_1_006_2" }, { "id": "mc_fir_4_014_2" }, { "id": "mc_fir_2_011_2" }, { "id": "mc_pine_1_007_2" }, { "id": "mc_fir_1_008_2" }, { "id": "mc_fir_1_009_2" }, { "id": "mc_fir_1_010_2" }, { "id": "mc_fir_2_013_2" }, { "id": "mc_fir_2_015_2" }, { "id": "mc_fir_2_016_2" }, { "id": "mc_fir_2_018_2" }, { "id": "mc_fir_2_019_2" }, { "id": "mc_fir_4_015_2" }, { "id": "mc_fir_4_017_2" }, { "id": "mc_fir_4_018_2" }, { "id": "mc_fir_4_019_2" }, { "id": "mc_fir_4_021_2" }, { "id": "mc_fir_4_023_2" }, { "id": "mc_fir_4_026_2" }, { "id": "mc_pine_1_008_2" }, { "id": "mc_pine_1_009_2" }, { "id": "mc_pine_1_010_2" }],
        /*[{ "id": "est_loc_02" }, { "id": "mc_pine_1_001_2" }, { "id": "mc_fir_2_001_2" }, { "id": "mc_fir_4_001_2" }, { "id": "mc_fir_1_001_2" }, { "id": "mc_fir_4_003_2" }, { "id": "mc_fir_2_003_2" }, { "id": "mc_fir_1_002_2" }, { "id": "mc_fir_1_003_2" }, { "id": "mc_pine_1_002_2" }, { "id": "mc_fir_2_004_2" }, { "id": "mc_fir_4_004_2" }, { "id": "mc_fir_4_005_2" }, { "id": "mc_fir_4_006_2" }, { "id": "mc_fir_4_007_2" }, { "id": "mc_fir_1_004_2" }, { "id": "mc_plant_116_004_2" }, { "id": "mc_plant_101_004_2" }, { "id": "mc_plant_101_003_2" }, { "id": "mc_plant_101_005_2" }, { "id": "mc_plant_103_002_2" }, { "id": "mc_plant_103_001_2" }, { "id": "mc_plant_101_006_2" }, { "id": "mc_plant_116_005_2" }, { "id": "mc_plant_101_001_2" }, { "id": "mc_fir_2_006_2" }, { "id": "mc_fir_4_008_2" }, { "id": "mc_fir_2_007_2" }, { "id": "mc_pine_1_003_2" }, { "id": "mc_plant_101_002_2" }, { "id": "mc_fir_4_009_2" }, { "id": "mc_fir_4_010_2" }, { "id": "mc_pine_1_004_2" }, { "id": "mc_fir_1_005_2" }, { "id": "mc_fir_4_012_2" }, { "id": "mc_fir_4_013_2" }, { "id": "est_loc_02_collider" }],*/
        z: 20,
        textures: ['pokazyvalka.png', 'pokazyvalka2.png', '3d_icon_downstairs.png', '3d_icon_enter.png', '3d_icon_exit.png', '3d_icon_upstairs.png', 'area_room_white.png', 'lenghtarrow.png', 'dotlength.png'],
        scripts: ['houses/main.js', 'p2player.js', 'babylonjs/TessellateModifier.js', 'babylonjs/SubdivisionModifier.js', 'houses/EffectComposer.js', 'houses/HorizontalBlurShader.js', 'houses/VerticalBlurShader.js', 'houses/RenderPass.js', 'houses/ShaderPass.js', 'houses/CopyShader.js', 'houses/SimplifyModifier.js']
    ...
    31: {
        ...
        mobileLook: -10,
        textures: ['pokazyvalka.png', 'pokazyvalka2.png', '3d_icon_downstairs.png', '3d_icon_enter.png', '3d_icon_exit.png', '3d_icon_upstairs.png', 'area_room_white.png', 'lenghtarrow.png', 'dotlength.png'],
        scripts: ['houses/main.js', 'p2player.js', 'houses/EffectComposer.js', 'houses/HorizontalBlurShader.js', 'houses/VerticalBlurShader.js', 'houses/RenderPass.js', 'houses/ShaderPass.js', 'houses/CopyShader.js', 'Reflector.js', 'houses/SimplifyModifier.js']
    },
    ...
    35: {
        ...
        textures: ['pokazyvalka.png', 'pokazyvalka2.png', '3d_icon_downstairs.png', '3d_icon_enter.png', '3d_icon_exit.png', '3d_icon_upstairs.png', 'area_room_white.png', 'lenghtarrow.png', 'dotlength.png', 'area_room_white.png'],
        scripts: ['houses/main.js', 'p2player.js', 'houses/EffectComposer.js', 'houses/HorizontalBlurShader.js', 'houses/VerticalBlurShader.js', 'houses/RenderPass.js', 'houses/ShaderPass.js', 'houses/CopyShader.js', 'houses/SimplifyModifier.js']
    },
    ...
    39: {
        ...
        textures: ['pokazyvalka.png', 'pokazyvalka2.png', '3d_icon_downstairs.png', '3d_icon_enter.png', '3d_icon_exit.png', '3d_icon_upstairs.png', 'area_room_white.png', 'area_room_white.png'],
        scripts: ['houses/main.js', 'p2player.js', 'houses/EffectComposer.js', 'houses/HorizontalBlurShader.js', 'houses/VerticalBlurShader.js', 'houses/RenderPass.js', 'houses/ShaderPass.js', 'houses/CopyShader.js', 'real_mirror.js', 'FXAAShader.js', 'houses/SimplifyModifier.js']
    },
    ...
    40: {
        ...
        obj: [],
        z: 50,
        textures: ['pokazyvalka.png', 'pokazyvalka2.png', '3d_icon_downstairs.png', '3d_icon_enter.png', '3d_icon_exit.png', '3d_icon_upstairs.png', 'lenghtarrow.png', 'dotlength.png', 'area_room_white.png', 'lenghtarrow.png', 'dotlength.png', 'area_room_white.png'],
        scripts: ['houses/main.js', 'p2player.js', 'houses/EffectComposer.js', 'houses/HorizontalBlurShader.js', 'houses/VerticalBlurShader.js', 'houses/RenderPass.js', 'houses/ShaderPass.js', 'houses/CopyShader.js', 'real_mirror.js', 'FXAAShader.js', 'houses/SimplifyModifier.js']
    },
    ...
    72: {
        ...
        //"z": 20,
        scripts: ['sky/sky.js', 'sky/see.js', 'sky/thunderbolt.js'],
        onInit: function() {
            this.sky = new Sky();
            ...
            this.see = new See2();
            //chat3d.world.renderer.autoClear = false;

            this.thunder = new Thunderbolt();
            chat3d.world.obj.push(this.thunder.mesh);
            chat3d.world.scene.add(this.thunder.mesh);
            this.thunder.mesh.rotation.y = Math.PI;
            this.thunder.mesh.position.z = 2500;
            this.thunder.mesh.position.y = 350;
            this.thunder.mesh.position.x = 1450;
            this.thunder.mesh.visible = false;


            this.thunder2 = new Thunderbolt();
            chat3d.world.obj.push(this.thunder2.mesh);
            chat3d.world.scene.add(this.thunder2.mesh);
            //this.thunder2.mesh.rotation.y = Math.PI;
            this.thunder2.mesh.position.z = -3500;
            this.thunder2.mesh.position.y = 350;
            this.thunder2.mesh.position.x = 150;
            this.thunder2.mesh.visible = false;

            let ambientLight = new BABYLON.AmbientLight(0x111111);
            chat3d.world.scene.add(ambientLight);
            chat3d.world.obj.push(ambientLight);

            /*let pointLight = new BABYLON.PointLight(0xd9d29d, 1, 0);
            pointLight.position.y = 10;
            chat3d.world.scene.add(pointLight);
            chat3d.world.obj.push(pointLight);*/

            let spotlight = new BABYLON.SpotLight(0xd2cfb9, 0.3, 0);
            console.log(spotlight);
            this.spotlight = spotlight;

            spotlight.position.set(1200, 2000, 5000);
            chat3d.world.scene.add(spotlight);
            chat3d.world.obj.push(spotlight);

            let dirlight = new BABYLON.DirectionalLight(0xd2cfb9, 0.5);
            dirlight.position.set(0, 1000, 1);
            chat3d.world.scene.add(dirlight);
            chat3d.world.obj.push(dirlight);
            ...
            this.splashTime = Math.random() * 4 + 3;

            this.splashCount = 1 + Math.floor(Math.random() * 3);
        ...
        onRender: function(delta) {
            //delta /= 10;
            this.time += delta;
            /*$('#debug_time').text(this.time);*/
            this.thunder.update(delta);
            this.thunder2.update(delta);

            this.sky.update(delta);
            //this.relf_sky.update(delta);
            this.see.update(delta);
            ...
            if (this.splash) {
                ...
                if (this.time > this.splashTime) {
                    this.splash = true;
                    this.time = 0;
                    this.thunder.mesh.visible = true;
                    this.thunder2.mesh.visible = true;
                    this.thunder.mesh.position.x = 1450 - 1600 * Math.random();
                    this.thunder2.mesh.position.x = 150 - 1300 * Math.random();
                    ...
                    if (this.splashCount == 0) {
                        ...
                        this.splashTime = 0.2 * Math.random();
                    }
                    //console.log(this.sky);
                }
            }

            //chat3d.world.renderer.clear();
            //return true;

        },

        onClean: function() {
            // chat3d.world.renderer.autoClear = true;
        },
    ...
    74: {
        ...
        css: ['speed.css'],
        html: ['speed.html'],
        scripts: ['hoverboard/character.js', 'p2player.js', 'town/SimplifyModifier.js', 'town/main.js'],
        textures: [
            'hoverboard_001.jpg',
            'block10_merged_mat_ao.jpg', 'block7_merged-mat-_MainTex-atlas0.jpg', 'Vehicle_Car_2.jpg',
            'block10_merged-mat-_MainTex-atlas0.jpg', 'block8_merged_mat_ao.jpg', 'Vehicle_Car_3.jpg',
            'block11_merged_mat_ao.jpg', 'block8_merged-mat-_MainTex-atlas0.jpg', 'Vehicle_Container_1.jpg',
            'block11_merged-mat-_MainTex-atlas0.jpg', 'block9_merged_mat_ao.jpg', 'Vehicle_Container_2.jpg',
            'block1_ao.jpg', 'block9_merged-mat-_MainTex-atlas0.jpg', 'Vehicle_Container_3.jpg',
            'block1_merged-mat-_MainTex-atlas0.jpg', 'intersection_03_merged-mat-_MainTex-atlas0.jpg', 'Vehicle_Pick_up_Truck_1.jpg',
            'block2_ao.jpg', 'park2_merged_mat_ao.jpg', 'Vehicle_Pick_up_Truck_2.jpg',
            'block2_merged-mat-_MainTex-atlas0.jpg', 'park2_merged-mat-_MainTex-atlas0.jpg', 'Vehicle_Pick_up_Truck_3.jpg',
            'block3_merged_mat_ao.jpg', 'park_3_ao.jpg', 'Vehicle_Police_Car.jpg',
            'block3_merged-mat-_MainTex-atlas0.jpg', 'park_merged-mat-_MainTex-atlas0.jpg', 'Vehicle_SUV_1.jpg',
            'block4_merged_mat_ao.jpg', 'Props_Props_01.jpg', 'Vehicle_SUV_2.jpg',
            'block4_merged-mat-_MainTex-atlas0.jpg', 'Road.jpg', 'Vehicle_SUV_3.jpg',
            'block5_merged_mat_ao.jpg', 'Vehicle_Ambulance.jpg', 'Vehicle_Taxi.jpg',
            'block5_merged-mat-_MainTex-atlas0.jpg', 'Vehicle_Bus_1.jpg', 'Vehicle_Truck_1.jpg',
            'block6_merged_mat_ao.jpg', 'Vehicle_Bus_2.jpg', 'Vehicle_Truck_2.jpg',
            'block6_merged-mat-_MainTex-atlas0.jpg', 'Vehicle_Bus_3.jpg', 'Vehicle_Truck_3.jpg',
            'block7_merged_mat_ao.jpg', 'Vehicle_Car_1.jpg', 'vignetting.png',
        ],
        geo: ['hoverboard_hoverboard.js', 'block_10_merged.js', 'Cloud_Cumulus_Med.js', 'Vehicle_Container_color01.js',
            'block_11_merged.js', 'park_2_merged.js', 'Vehicle_Container_color02.js',
            'block_1_merged.js', 'park_3_merged.js', 'Vehicle_Container_color03.js',
            'block_2_merged.js', 'Road_Intersection_03_merged_fixed.js', 'Vehicle_Pick_up_Truck_color01.js',
            'block_3_merged.js', 'Road_Intersection_04_fixed.js', 'Vehicle_Pick_up_Truck_color02.js',
            'block_4_merged.js', 'Road_Intersection_05.js', 'Vehicle_Pick_up_Truck_color03.js',
            'block_5_merged.js', 'road_lane_01_fixed.js', 'Vehicle_Police_Car.js',
            'block_6_merged.js', 'road_lane_03_fixed.js', 'Vehicle_SUV_color01.js',
            'block_7_merged.js', 'Vehicle_Ambulance.js', 'Vehicle_SUV_color02.js',
            'block_8_merged.js', 'Vehicle_Bus_color01.js', 'Vehicle_SUV_color03.js',
            'block_9_merged.js', 'Vehicle_Bus_color02.js', 'Vehicle_Taxi.js',
            'Cloud_Cumulus_(1).js', 'Vehicle_Bus_color03.js', 'Vehicle_Truck_color01.js',
            'Cloud_Cumulus_(2).js', 'Vehicle_Car_color01.js', 'Vehicle_Truck_color02.js',
            'Cloud_Cumulus.js', 'Vehicle_Car_color02.js', 'Vehicle_Truck_color03.js',
            'Cloud_Cumulus_Med_(1).js', 'Vehicle_Car_color03.js',
            'town_col_0.js',
            'town_col_10.js',
            'town_col_11.js',
            'town_col_12.js',
    ...
    75: {
        obj: [],
        z: -183,
        scripts: ['hall/shaders.js', 'hall/BufferGeometryUtils.js', 'hall/BufferGeometryUtils.js', 'hall/Wagner.js', 'hall/Wagner.base.js', 'hall/ShaderLoader.js', 'hall/main.js'],
        geo: ['polySurface7.js'],
        textures: ['hall_baked_4096.jpg', 'hall_spec_4096.jpg', 'hall_norm_4096.jpg', 'hall_displacement.jpg'],
        //disableCharOptimization: true,
        //showSelf: true,
    ...
    76: {
        ...
        z: 20,
        textures: ['pokazyvalka.png', 'pokazyvalka2.png', 'waternormals.jpg', 'Water_1_M_Normal.jpg', 'Water_2_M_Normal.jpg'],
        scripts: ['p2player.js', 'horMirror.js', 'Water2.js', 'street.js']
    ...
    77: {
        ...
        obj: [{ "id": "loc_18e" }, { "id": "loc_18e_collider" }, { "id": "loc_18_mc_tree_1_001" }, { "id": "loc_18_mc_plant_111_001" }, { "id": "loc_18_mc_plant_116_001" }, { "id": "loc_18_mc_plant_201_001" }, { "id": "loc_18_mc_plant_208_001" }, { "id": "loc_18_mc_tree_3_001" }, { "id": "loc_18_mc_tree_3_002" }, { "id": "loc_18_mc_tree_5_002" }, { "id": "loc_18_mc_tree_1_002" }, { "id": "loc_18_mc_tree_2_001" }, { "id": "loc_18_mc_tree_2_002" }, { "id": "loc_18_mc_tree_5_001" }, { "id": "loc_18_mc_plant_201_002" }, { "id": "loc_18_mc_plant_201_003" }, { "id": "loc_18_mc_tree_4_001" }, { "id": "loc_18_mc_tree_1_003" }, { "id": "loc_18_mc_plant_101_001" }, { "id": "loc_18_mc_plant_101_002" }, { "id": "loc_18_mc_plant_101_003" }, { "id": "loc_18_mc_plant_204_001" }, { "id": "loc_18_mc_plant_116_002" }, { "id": "loc_18_mc_plant_103_001" }, { "id": "loc_18_mc_plant_103_002" }, { "id": "loc_18_mc_plant_103_003" }, { "id": "loc_18_mc_plant_201_004" }, { "id": "loc_18_mc_plant_103_004" }, { "id": "loc_18_mc_plant_111_002" }, { "id": "loc_18_mc_plant_103_005" }, { "id": "loc_18_mc_plant_214_001" }, { "id": "loc_18_mc_plant_116_004" }, { "id": "loc_18_mc_plant_208_002" }, { "id": "loc_18_mc_plant_201_005" }, { "id": "loc_18_mc_plant_k13_001" }, { "id": "loc_18_mc_plant_305_001" }, { "id": "loc_18_mc_plant_k03_001" }, { "id": "loc_18_mc_plant_k05_001" }, { "id": "loc_18_mc_plant_305_002" }, { "id": "loc_18_mc_plant_116_003" }, { "id": "loc_18_mc_plant_k13_002" }, { "id": "loc_18_mc_plant_305_003" }, { "id": "loc_18_mc_plant_k05_002" }, { "id": "loc_18_mc_plant_305_004" }, { "id": "loc_18_mc_plant_116_005" }, { "id": "loc_18e_ground" }],
        z: 20,
        textures: ['pokazyvalka.png', 'pokazyvalka2.png', '3d_icon_downstairs.png', '3d_icon_enter.png', '3d_icon_exit.png', '3d_icon_upstairs.png', 'area_room_white.png', , 'lenghtarrow.png', 'dotlength.png'],
    ...
    79: {
        "obj": [{ "id": "loc_09est_hills_02" }, { "id": "loc_09est_floor_02" }, { "id": "loc_09_collider_02" }, { "id": "mc_fir_4_003_02" }, { "id": "mc_fir_2_003_02" }, { "id": "mc_fir_4_004_02" }, { "id": "mc_plant_116_004_02" }, { "id": "mc_plant_101_004_02" }, { "id": "mc_plant_101_003_02" }, { "id": "mc_plant_103_001_02" }, { "id": "mc_plant_101_006_02" }, { "id": "mc_plant_116_005_02" }, { "id": "mc_plant_101_001_02" }],
        "z": 20,
        skybox: {
            w: 80000,
            h: 50000,
    ...
    80: {
        ...
        textures: ['pokazyvalka.png', 'pokazyvalka2.png', '3d_icon_downstairs.png', '3d_icon_enter.png', '3d_icon_exit.png', '3d_icon_upstairs.png', 'area_room_white.png', 'lenghtarrow.png', 'dotlength.png', 'area_room_white.png'],
        scripts: ['houses/main.js', 'p2player.js', 'babylonjs/TessellateModifier.js', 'babylonjs/SubdivisionModifier.js', 'houses/EffectComposer.js', 'houses/HorizontalBlurShader.js', 'houses/VerticalBlurShader.js', 'houses/RenderPass.js', 'houses/ShaderPass.js', 'houses/CopyShader.js', 'real_mirror.js', 'FXAAShader.js', 'houses/SimplifyModifier.js']
    ...
    81: {
        ...
        scripts: ['hyper-mix/math.js', 'hyper-mix/fboHelper.js', 'hyper-mix/simulator.js', 'hyper-mix/settings.js', 'hyper-mix/volume.js', 'hyper-mix/lights.js', 'hyper-mix/particles.js', 'hyper-mix/floor.js', 'hyper-mix/postprocessing.js', 'hyper-mix/main.js']

    },


    82: {
        "obj": [{ "id": "est_loc_01" }, { "id": "est_loc_01" }, { "id": "mc_fir_1_001" }, { "id": "mc_fir_1_001" }, { "id": "mc_pine_3_001" }, { "id": "mc_pine_3_001" }, { "id": "mc_pine_2_002" }, { "id": "mc_pine_2_002" }, { "id": "mc_fir_2_005" }, { "id": "mc_fir_2_005" }, { "id": "mc_fir_2_004" }, { "id": "mc_fir_2_004" }, { "id": "mc_fir_4_001" }, { "id": "mc_fir_4_001" }, { "id": "mc_fir_2_001" }, { "id": "mc_fir_2_001" }, { "id": "mc_fir_2_002" }, { "id": "mc_fir_2_002" }, { "id": "mc_pine_3_003" }, { "id": "mc_pine_3_003" }, { "id": "mc_pine_2_005" }, { "id": "mc_pine_2_005" }, { "id": "mc_fir_2_006" }, { "id": "mc_fir_2_006" }, { "id": "mc_pine_1_004" }, { "id": "mc_pine_1_004" }, { "id": "mc_pine_3_008" }, { "id": "mc_pine_3_008" }, { "id": "mc_pine_3_005" }, { "id": "mc_pine_3_005" }, { "id": "mc_fir_3_005" }, { "id": "mc_fir_3_005" }, { "id": "mc_fir_4_003" }, { "id": "mc_fir_4_003" }, { "id": "mc_fir_3_004" }, { "id": "mc_fir_3_004" }, { "id": "mc_fir_3_003" }, { "id": "mc_fir_3_003" }, { "id": "mc_fir_3_002" }, { "id": "mc_fir_3_002" }, { "id": "mc_pine_2_006" }, { "id": "mc_pine_2_006" }, { "id": "mc_pine_1_003" }, { "id": "mc_pine_1_003" }, { "id": "mc_pine_1_002" }, { "id": "mc_pine_1_002" }, { "id": "mc_pine_3_002" }, { "id": "mc_pine_3_002" }, { "id": "mc_pine_1_001" }, { "id": "mc_pine_1_001" }, { "id": "est_loc_01_collider" }, { "id": "est_loc_01_collider" }],
        "z": 20,
        skybox: {
            w: 80000,
            h: 50000,
    ...
    84: {
        ...
        noSlip: true,
        scripts: ['crossstreet/main.js']

    },

    85: {
        "obj": [{ "id": "est_loc_06_pine_tree_001" }, { "id": "est_loc_06" }, { "id": "est_loc_06_pine_bush_bgr" }, { "id": "plants_7_19" }, { "id": "plants_7_20" }, { "id": "pine_tree_002" }, { "id": "est_loc_06_collider" }],
        "z": 20,
        skybox: {
            w: 80000,
            h: 50000,
    ...
    87: {
        "obj": [{ "id": "est_loc_06_sphere" }, { "id": "est_loc_06_pine_tree_001" }, { "id": "est_loc_06" }, { "id": "est_loc_06_bush_bgr" }, { "id": "plants_7_19" }, { "id": "plants_7_20" }, { "id": "pine_tree_002" }, { "id": "est_loc_06_collider" }],
        "z": 20,
        textures: ['pokazyvalka.png', 'pokazyvalka2.png', '3d_icon_downstairs.png', '3d_icon_enter.png', '3d_icon_exit.png', '3d_icon_upstairs.png', 'area_room_white.png', 'lenghtarrow.png', 'dotlength.png'],
        scripts: ['houses/main.js', 'p2player.js', 'babylonjs/TessellateModifier.js', 'babylonjs/SubdivisionModifier.js', 'houses/EffectComposer.js', 'houses/HorizontalBlurShader.js', 'houses/VerticalBlurShader.js', 'houses/RenderPass.js', 'houses/ShaderPass.js', 'houses/CopyShader.js', 'houses/SimplifyModifier.js']
    ...
    89: {
        obj: [],
        z: -90,
        scripts: ['bil/cannon.min.js', 'bil/main.js', 'p2player.js'],
        textures: ['1ball.png', '2ball.png', '3ball.png', '4ball.png', '5ball.png', '6ball.png', '7ball.png', '8ball.png', '9ball.png', '10ball.png', '11ball.png', '12ball.png', '13ball.png', '14ball.png', '15ball.png', 'whiteball.png', 'Cue.jpg'],
        geo: ['base.json', 'edges.json', 'felt.json', 'pocket_bottoms.json', 'pockets.json', 'stick.json', 'tip.json', 'cue01.js'],
    },

    90: {
        obj: [{ "id": "tree_13a_001_Mesh" }, { "id": "est_loc_07_Mesh.001" }, /*{ "id": "water_Mesh.002" },*/ { "id": "mc_plant_201_Mesh.003" }, { "id": "mc_plant_202_001_Mesh.004" }, { "id": "mc_plant_202_002_Mesh.005" }, { "id": "pool_bottom_Mesh.006" }, { "id": "est_loc_07_collider_Mesh.007" }, { "id": "est_loc_06_sphere", pos: [-700, 0, 700] }, ],
        z: 20,
        textures: ['pokazyvalka.png', 'pokazyvalka2.png', '3d_icon_downstairs.png', '3d_icon_enter.png', '3d_icon_exit.png', '3d_icon_upstairs.png', 'area_room_white.png', 'lenghtarrow.png', 'dotlength.png', 'Water_1_M_Normal.jpg', 'Water_2_M_Normal.jpg', 'water_normal_1.jpg'],
        geo: ['water_Mesh.002.js', 'pool_bottom_Mesh.006.js'],
        scripts: ['houses/main.js', 'p2player.js', 'babylonjs/TessellateModifier.js', 'babylonjs/SubdivisionModifier.js', 'houses/EffectComposer.js', 'houses/HorizontalBlurShader.js', 'houses/VerticalBlurShader.js', 'houses/RenderPass.js', 'houses/ShaderPass.js', 'houses/CopyShader.js', 'houses/SimplifyModifier.js', 'Water2.js']


    },

    92: {
        obj: [{ "id": "Cube.001_Cube.002" }, { "id": "Cube.008_Cube.003" }, { "id": "Cube.028_Cube.035" }, { "id": "cKey_Cube.004" }, { "id": "vKey_Cube.006" }, { "id": "keyboard_Cube.007" }, { "id": "hKey_Cube.009" }, { "id": "eKey_Cube.010" }, { "id": "yKey_Cube.011" }, { "id": "ctrlKey_Cube.012" }, { "id": "spaceKey_Cube.014" }, { "id": "Cube_Cube.001" }, /*{ "id": "coffeSteam_Plane.001" },*/ { "id": "eminemBack_Plane.003" }, { "id": "eminem_Plane.005" }, { "id": "topbenchPress_Plane.006" }, { "id": "chairTop_Cube.005" }, { "id": "mirror_Plane.004" }, { "id": "panel_Plane.002" }, { "id": "text_Text" }, { "id": "flowers_Cylinder.013" }, { "id": "mog_Cylinder" }, { "id": "wall1" }, { "id": "wall2" }, { "id": "wall3" }, { "id": "loc92_col2" }, { "id": "loc92_col1" }, { "id": "loc92_col3" }, { "id": "loc92_col5" }, { "id": "loc92_col4" }, { "id": "loc92_col6" }, { "id": "loc_col7" }, { "id": "loc92_col8" }, { "id": "loc92_col9" }, { "id": "loc92_col10" }, { "id": "Plane_1" }],
        z: -40,
        onInit: function() {
            //79.79656219482422, y: 192.2710418701172, z: 6.1513519287109375
            let mesh = new BABYLON.Realmirror(80, 192.271);
            this.mirror = mesh;
            //-337.08547592163086, y: 41.376800537109375, z: 287.9763984680176
            mesh.position.x = -337.08;
            mesh.position.y = 41.376;
            mesh.position.z = 287.5;
            mesh.rotation.y = Math.PI;
            console.log(mesh.rotation.x);
            mesh.rotation.x = 0.032;
            chat3d.world.obj.push(mesh);
            chat3d.world.scene.add(mesh);
        },
    ...
    93: {
        obj: [{ "id": "loc93_Cube320" }, { "id": "loc93_top001" }, { "id": "loc93_Plane034" }, { "id": "loc93_Plane013" }, /*{ "id": "loc93_Cube318" },*/ { "id": "loc93_screen001" }, { "id": "loc93_screen007" }, { "id": "loc93_screen009" }, { "id": "loc93_screen011" }, { "id": "loc93_screen003" }, { "id": "loc93_screen005" }, { "id": "loc93_screen013" }, { "id": "loc93_screen004" }, { "id": "loc93_screen002" }, { "id": "loc93_screen006" }, { "id": "loc93_screen010" }, { "id": "loc93_screen012" }, { "id": "loc93_screen008" }, { "id": "loc93_screen014" }, /*, { "id": "loc93_led2" }, { "id": "loc93_led3" }, { "id": "loc93_led1" }, { "id": "loc93_led0" }*/ { "id": "loc93_floor" }, { "id": "loc93_walls" }, { "id": "loc93_walls2" }, /*{ "id": "wall3" }*/ { "id": "loc93_Cube271" },
            { "id": "loc93_col1" }, { "id": "loc93_col2" }, { "id": "loc93_col3" }, { "id": "loc93_col4" }, { "id": "loc93_room_tv" }, { "id": "night_city" }, { "id": "roof" }
        ],
        z: 20,
        onInit: function() {
            //console.log(chat3d.world.obj);
            let floor;
            for (let i in chat3d.world.obj) {
                if (chat3d.world.obj[i].name == 'loc93_floor') {
                    floor = chat3d.world.obj[i];
                }
            }
            console.log(floor);
            let box = new BABYLON.Box3();
            box.setFromObject(floor);
            var shadowScale = 1.15;
            var width = box.getSize().x;
            var height = box.getSize().z;
            var camera = new BABYLON.OrthographicCamera(width / -2, width / 2, height / 2, height / -2, 1, 195);
            chat3d.world.shadow_uniforms.shadowCenter.value.x = box.getCenter().x;
            chat3d.world.shadow_uniforms.shadowCenter.value.y = box.getCenter().z;
            chat3d.world.shadow_uniforms.shadowSize.value.x = box.getSize().x;
            chat3d.world.shadow_uniforms.shadowSize.value.y = box.getSize().z;
            camera.position.copy(box.getCenter());
            camera.position.y += 200;
            camera.lookAt(box.getCenter());

            var clearColor = chat3d.world.renderer.getClearColor().clone();

            chat3d.world.renderer.setClearColor(0xffffff);

            //chat3d.world.scene.overrideMaterial = new BABYLON.MeshBasicMaterial({ color: 0x000000 });
            let rpass = new BABYLON.RenderPass(chat3d.world.scene, camera, new BABYLON.MeshBasicMaterial({ color: 0x777777 }));
            var composer = new BABYLON.EffectComposer(chat3d.world.renderer);
            //rpass.renderToScreen = true;
            composer.addPass(rpass);
            console.log(BABYLON.HorizontalBlurShader);
            //BABYLON.HorizontalBlurShader.uniforms.h.value = 0.5;
            var hblur = new BABYLON.ShaderPass(BABYLON.HorizontalBlurShader);
            composer.addPass(hblur);

            var vblur = new BABYLON.ShaderPass(BABYLON.VerticalBlurShader);
            //vblur.renderToScreen = true;
            composer.addPass(vblur);


            composer.render();
            //chat3d.world.scene.overrideMaterial = null;

            chat3d.world.renderer.setClearColor(clearColor);
            chat3d.world.shadow_uniforms.shadowMap.value = composer.renderTarget2.texture;

            console.log(chat3d.world.video);
            chat3d.world.video.preloader_mesh.scale.set(0.5, 0.5, 0.5);
        ...
        video: {
            ...
            x2: 200,
            y1: 0,
            loader_pos: [209, 135, 100],
            loader_rot: [0, -Math.PI / 2, 0]

        },
        scripts: ['houses/EffectComposer.js', 'houses/HorizontalBlurShader.js', 'houses/VerticalBlurShader.js', 'houses/RenderPass.js', 'houses/ShaderPass.js', 'houses/CopyShader.js']
...
//locations['12'] = locations[76];
locations.street = locations[12];

locations[83] = locations[82];

locations[91] = locations[90];

//locations[86] = locations[85];

//locations[88] = locations[87];