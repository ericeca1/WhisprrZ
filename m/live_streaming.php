...
class CLiveStreaming extends CHtmlBlock
{
	function parseBlock(&$html)
	{
		...
		if ($isPresenter){
			$pageUrl = Common::pageUrl('live', $guid);
		} else {
			if ($isStreamStart) {
				$pageUrl = Common::pageUrl('live_', $clientId, $liveView);
			} else {
				$pageUrl = Common::pageUrl('live_id', $clientId, $liveView);
			}
		}
        if (!$isPresenter && !$isStreamStart) {
			$pageUrl = Common::pageUrl('live_', $clientId, $liveView);
            $html->setvar('page_url_start', $pageUrl);
            $html->setvar('page_live_view_id', $liveView);
            $html->parse('viewer_auto_connect_js');
        }
		$html->setvar('page_url_history', $pageUrl);//Impact mobile
...
$tmpl = array('main' => $g['tmpl']['dir_tmpl_mobile'] . 'live_streaming.html',
			  'live_streaming' => $g['tmpl']['dir_tmpl_mobile'] . '_live_streaming.html');
