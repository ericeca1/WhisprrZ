...
function showAlert(msg,wr,vis,t,isCloseClickShadow){
	...
	tip.find('.tooltip').removeClass('blue red').addClass(vis);
...
function goToUserVideo(vid, uid, src, poster){
	...
	if (!userAllowedFeature['live_streaming'] && cprofile.guid != uid){
    ...
    $layer.addClass('to_show').addChildrenLoader();
	var url=urlPagesSite.profile_view+'?user_id='+uid+'&display=profile_info&video_show='+vid;
	redirectUrl(url);
}
...
function serverError(){
    showAlert(l('server_error_try_again'),'body');
}