...
class XlmGallery extends CHtmlBlock
{
            function parseBlock(&$html)
            {
                        ...
                        while($row = DB::fetch_row()) {
                        			if (!in_array($row['filename'], $ims) and custom_file_exists($g['path']['dir_files'] . 'gallery/images/' . $row['user_id'] . '/' . $row['folder'] . '/' . $row['filename'] . '') and custom_file_exists($g['path']['dir_files'] . 'gallery/thumb/' . $row['user_id'] . '/' . $row['folder'] . '/' . $row['filename'] . '')) {
                        						foreach ($row as $k => $v) {
                        									$html->setvar($k, $v);
                        						}
