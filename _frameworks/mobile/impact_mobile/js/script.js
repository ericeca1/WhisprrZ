...
var currentScriptSrcParts = document.currentScript.src.split('_frameworks/mobile/impact_mobile/js/script.js');
var _lsLiveStreamRecordSaveUrl = currentScriptSrcParts[0] + 'ajax.php';
...
$.fn.oneTransEnd_OLD=function(fn, prop){
	var el=this;
    el.on(transEvent, function f(e){
...
$(function(){
    ...
    $('body').on('click', '.go_to_page, #toTop, .popup_alert a, a.empty, a.timeline_photo_comment', function(e){
        var $el=$(this);
...
function uploadPageMember($link,url,back){
	...
	if($('.module_live_streaming')[0]){
		_lsMediaClose(true);
		_lsPresenterClosePpImpactMobile();
	}else if($('.module_media_chat')[0]){
		mediaChatClose();
	}
    updateStart=false;
    $.ajax({url:url,
            ...
            if ($data[0]) {
                ...
                if($main[0]){
                    ...
                    setTimeout(function(){
                        ...
                        hideLayerBlockPage();
                        $('.pp_shadow:not(.pp_shadow_live), .pp_wrapper:not(.pp_wrapper_live)').remove();
                        isUploadPage=false;
                    },250)
                }else{
                    if (typeof res == 'object' && res.redirect) {
        ...
        success: function(res){
            ...
            if($main[0]){
                ...
                var fn=function(){
                    ...
                    clearHistoryPage();
                    replaceScriptPage($data,true);
                    $body.removeClass('body_member');
                    $('html').removeClass('html_member');
                    $body.find('#main_wrap').replaceWith($main);
					setTimeout(function(){
						$body.addClass('body_visitor');
					},600)
                    siteGuid=0;
                    isUploadPage=false;
                    hideLayerBlockPage();
...
function showConfirm(msg, handler, hCancel, btnOk, btnCancel, onlyOk, notClose, alingCenter, fa, zIndex){
    ...
    return $pp.modalPopup({wrClass: 'alert_wrapper',css:{},wrCss:{zIndex:(zIndex||1001)}}).open()
}
function showConfirmToPage(msg, url, btnOk, onlyOk, fa){
    onlyOk=defaultFunctionParamValue(onlyOk,true);
    showConfirm(msg, function(){
        goToPage($('.pp_btn_ok_bl:visible').data('url',url));
    }, false, btnOk, '', onlyOk, true, false, fa);
}
...
function updateMenuCounterAll(data){
    for (var el in data) {
        ...
        if (el=='messages') {
			...
			updateCounter(el, newC, newC, 1, $('.counter_profile_visitors_header'));
        }else{
...
function goToLive(lid, url){
	...
	goToPage($('<a data-layer-loader="false" data-no-loader="true">'), url);
}

function goToUserVideo(vid, uid, src, poster){
    var $layer=$('#list_image_layer_action_'+vid);
    if($layer[0]&&$layer.is('.to_show'))return;

	if (!userAllowedFeature['live_streaming'] && clProfile.guid != uid){
		showConfirmToPage(l('watch_pasts_stream_need_upgrade'), urlPagesSite.upgrade, l('upgrade'), false, 'fa-exclamation-circle');
		return;
	}

    $layer.addClass('to_show').addChildrenLoader();
	var url=urlPagesSite.profile_view+'?user_id='+uid+'&video_show='+vid;
	goToPage($('<a data-layer-loader="false" data-no-loader="true">'), url);
}
