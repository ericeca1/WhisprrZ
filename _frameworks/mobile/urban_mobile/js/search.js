var CSearch = function() {
    ...
    this.pageBeforeUnload = function(){
        var page=$('#search_users_list');
        if(page[0]){
            /*var $sp=$('#spotlight'), hh=50;
            if($sp[0]){
                hh=50+$sp.height();
            }*/
            var hh=50,
                W=page.width(), H=$win.height()-hh, n=W>400?5:(W>300?4:3),
                cel=$('li:visible', page);
            if (cel[0]) {
                n=Math.floor(W/cel.width());
            }

            var h=cel.height(),
                offset=Math.round(($this.minOffset-1)/n-(page.offset().top-hh)/h)*n,
    ...
    $(function(){
        ...
        if ($this.display == 'encounters') {
            ...
            function setCookiePage(){
                ...
                var hh=50,
                    W=$win.width(), H=$win.height()-hh, n=W>400?5:(W>300?4:3),
                    cel=$('li:visible', page);
                if (cel[0]) {
                    n=Math.floor(W/cel.width());
                }
