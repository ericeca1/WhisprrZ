var CProfilePhoto = function(uid) {
                    ...
                    if (data!==false){
                        ...
                        img.onload = function(){
                            ...
                            if($this.galleryPhotosInfo[pid]['gif']) {
                                $cnt.find('.icon.rotate').parent().hide();
                            }
    ...
    this.showLinkActionPhoto = function(info){
        if($this.uid==$this.guid){
			...
			$this.$btnAdditional[info['gif']?'removeClass':'addClass']('to_show');
        }else{
    ...
    this.activateSwipeGallery = function() {
        ...
        if(/profile|encounters/.test($this.display)) $(selSwipe).css({overflow: 'hidden'}).swipe({
                ...
                excludedElements: 'button, a, #photo_gallery_set_default, #photo_gallery_report, #request_access, .pp_gallery_restore_image',
	...
	this.refreshImage = function(pid, edit, restore){
		if($this.galleryPhotosInfo[pid] != undefined){
            ...
            $this.replacePhotoAfterEditor(pid);
		}
	}
	...
	this.restoreImage = function($btn){
		...
		showConfirm(l('all_changes_will_be_lost'), fn)
	}
	...
	this.initImageEditor = function() {
		if ($this.guid == $this.uid) {
			initEditorImage('impact_mobile');
		}
	}
