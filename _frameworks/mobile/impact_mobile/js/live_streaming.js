var CLiveStreaming = function(guid) {
    ...
    this.confirmDeleteComment = function($el,cid,rCid){
        showConfirm(l('are_you_sure'), function(){$this.commentDelete($el,cid,rCid)}, l('confirm_delete_comment'));
    }
    ...
    this.searchLive = function($el, prev){
        ...
        $.post(url_ajax+'?cmd=live_search&view=mobile',
            ...
            function(res){
                ...
                if (data !== false){
                    if (data) {
						goToPage($('<a data-layer-loader="true">'), data);
                        //redirectUrl(data);
                    } else {
                        $el.removeChildrenLoader();
                        $bl.removeClass('to_show');
						showAlert(l('no_more_live_broadcasts_currently'),true,'fa-info-circle');
                    }
                    /* Test */
                    //$bl.removeClass('disabled');
                    //$el.removeChildrenLoader();
