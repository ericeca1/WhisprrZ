...
class CPartnerJoinForm extends CHtmlBlock
{
	...
	function action()
	{
		...
		if ($cmd == "login")
		{
            ...
            $user = DB::one('partner', '`name` = ' . to_sql($login, 'Text') . ' OR `mail` = ' . to_sql($login, 'Text'));
			...
			if ($user) {
				set_session("partner_id", $user['partner_id']);
		...
		elseif ($cmd == "join")
		{
			...
			if (DB::result("SELECT partner_id FROM partner WHERE name=" . to_sql($name) . ";") != 0)
			{
				if($this->jmessage =="") $this->jmessage .= l('username_exists')." ".l("choose_another");
			}

			if (DB::result("SELECT partner_id FROM partner WHERE mail=" . $mail . ";") != 0)
			{
			...
			if ($this->jmessage == "")
			{
				...
				DB::execute("
					...
					" . to_sql($name) . ",
					" . $partner . ",
					" . to_sql(User::preparePasswordForDatabase($pass)) . ",
					" . $company_name . ",
					" . $referring_domains . ",
					" . $contact_name . ",
					" . $phone . ",
