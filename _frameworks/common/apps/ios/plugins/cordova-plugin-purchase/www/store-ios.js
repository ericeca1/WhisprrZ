...
function defer(thisArg, cb, delay) {
	//console.log('defer START', thisArg, cb, delay);
    setTimeout(function() {
        cb.call(thisArg);
    }, delay || 1);
}
...
store.Product.prototype.verify = function() {
    ...
    var tryValidation = function() {
        ...
        store._validator(that, function(success, data) {
            if (!data) data = {};
			console.log('store._validator > ', data);
            ...
            if (success) {
                ...
                for(var customFieldIndex in customFields) {
                    ...
                    var customFieldValue = getData(data, customField);
                    if(customFieldValue) {
                        that[customField] = customFieldValue;
                    }
                }
                ...
                if (data.code === store.PURCHASE_EXPIRED) {
                    ...
                    nRetry += 1;
					console.log('validation retry', nRetry);
...
store.order = function(pid, additionalData) {
    ...
    store.ready(function() {
        console.log('store.order set state store.REQUESTED');
        p.push({
            state: store.REQUESTED
        });
    });

    console.log('store.order.cb return');
    ...
    return {
        ...
        then: function(cb) {
            console.log('store.order.cb start');
            console.log('store.order.cb value', JSON.stringify(cb));
            ...
            console.log('store.order.cb end');
        ...
        error: function(cb) {
            console.log('store.order.cb error');
            console.log('store.order.cb error value', JSON.stringify(cb));
...
/*		if('transaction' in product && 'appStoreReceipt' in product.transaction) {
			// check what is product type here and list of product.transactions > check finished receipt only if no transactions in wait list
			if(!('transactions' in product) && localStorage['iap_ ' + store.utils.md5(product.transaction.appStoreReceipt)] === '1') {
				console.log('store-ios.js > runValidation return appStoreReceipt exists in localStorage');
					data = {ok: true, data: []};
				...
				console.log('store-ios.js > runValidation appStoreReceipt NOT exists in localStorage', store.utils.md5(product.transaction.appStoreReceipt));
			}
		}*/

		if(MobileApp.iapIsReceiptFinished(product)) {
			console.log('store-ios.js > runValidation exit > iapIsReceiptFinished');
			data = {ok: true, data: []};
		    request.callbacks.forEach(function(callback) {
			    callback(data && data.ok, data.data);
			});			
...
/*		var iapPrefix = 'verifyPurchase';
		...
		if(MobileApp.iapIsTransactionInProcess(iapPrefix, product)) {
			console.log('store-ios.js > runValidation exit > iapIsTransactionInProcess');
			return;
		} else {
			MobileApp.iapAddTransactionInProcess(iapPrefix, product);
			console.log('store-ios.js > runValidation iapAddTransactionInProcess');
		}*/
      ...
      // Post
	  console.log('runValidation > store.utils.ajax');

      store.utils.ajax({
          url: new URL((typeof store.validator === 'string') ? store.validator : store.validator.url, document.baseURI).href,
          method: 'POST',
          ...
          error: function(status, message, data) {
              ...
              store.log.debug("body ERROR => " + data);
              request.callbacks.forEach(function(callback) {
        ...
        xhr.onreadystatechange = function(/*event*/) {
            try {
                if (xhr.readyState === 4) {
					...
					console.log('xhr.onreadystatechange', xhr.responseText, xhr);
...
InAppPurchase.prototype.purchase = function (productId, quantity, applicationUsername, discount) {
    ...
    console.log('store.order InAppPurchase.prototype.purchase', productId);
    ...
    console.log('store.order InAppPurchase.prototype.purchase exec', productId);
...
InAppPurchase.prototype.finish = function (transactionId) {
	console.log('InAppPurchase.prototype.finish', transactionId);
...
InAppPurchase.prototype.updatedTransactionCallback = function (state, errorCode, errorText, transactionIdentifier, productId, transactionReceipt, originalTransactionIdentifier) {
    ...
    if (productId && transactionIdentifier) {
        log("product " + productId + " has a transaction in progress: " + transactionIdentifier + " > STATE > " + state);
...
InAppPurchase.prototype.loadReceipts = function (callback, errorCb) {
	...
	log('loading appStoreReceipt > ' + Date());
...
store.when("requested", function(product) {
    console.log('store.order when requested');
    store.ready(function() {
        if (!product) {
            store.error({
...
function storekitFinish(product) {
	console.log('storekitFinish START', product.type, JSON.stringify(product));
    if (product.type === store.CONSUMABLE || product.type === store.NON_RENEWING_SUBSCRIPTION) {
		console.log('storekitFinish type', product.type);
        ...
        if (transactionId) {
			...
			console.log('storekitFinish transaction', transactionId);
        ...
        for (var i = 0; i < product.transactions.length; ++i) {
			...
			console.log('storekitFinish transaction finished', product.transactions[i]);
        }
...
function storekitSetAppProductFromReceipt(data) {
	//console.log('storekitSetAppProductFromReceipt', Date(), data);
    if (data) {
...
store._prepareForValidation = function(product, callback) {
    ...
    function loadReceipts() {
        console.log('store._prepareForValidation > loadReceipts', product.id, JSON.stringify(product.transactions));
        storekit.loadReceipts(function(data) {
            if (!product.transaction) {
                product.transaction = {
                    type: 'ios-appstore'
            ...
            if (!product.transaction.appStoreReceipt) {
                nRetry ++;
                if (nRetry < 2) {
                    console.log('store._prepareForValidation > setTimeout', nRetry);
