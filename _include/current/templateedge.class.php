...
Class TemplateEdge {
        ...
        if ($isFooter) {
            ...
            if($guid) {
                ...
                if(Common::isOptionActive('reports_approval')){
                   ...
                   $html->parse('video_report', false); //popcorn add 2024-01-26
                }
    ...
    static function parseCustomHeader(&$html, $uid = null)
    {
        ...
        if(self::isEHP()) {
            ...
            if($ehp_type == 'event') {
                ...
                $infoName = array('name' => $event['event_title'], 'age' => '');
                $nameShort = User::nameShort($event['event_title']);
                $infoBlock = array('name' => self::getUserName($infoName, $event_id),
                        'user_name' => $nameShort,
                        'user_name_attr' => toAttr($nameShort),
                        'user_name_title' => toAttr($event['event_title']),
                        'visible_name_short' => true,
                        'url' => "./events_event_show.php?event_id=" . $event_id
                );

                $event_main_images = CEventsTools::event_images($event_id);
                $url_files =$g['path']['url_files'];

                $photoMain = $event_main_images['image_file'];
                $photoMainR = $event_main_images['image_file'];
                $photoMainId = $event_main_images['photo_id'];
            } elseif ($ehp_type == 'hotdate') {
                $hotdate_id = self::getEHPId();
                $hotdate = ChotdatesTools::retrieve_hotdate_by_id($hotdate_id);

                $infoName = array('name' => $hotdate['hotdate_title'], 'age' => '');
                $nameShort = User::nameShort($hotdate['hotdate_title']);
                $infoBlock = array('name' => self::getUserName($infoName, $hotdate_id),
                        'user_name' => $nameShort,
                        'user_name_attr' => toAttr($nameShort),
                        'user_name_title' => toAttr($hotdate['hotdate_title']),
                        'visible_name_short' => true,
                        'url' => "./hotdates_hotdate_show.php?hotdate_id=" . $hotdate_id
                );

                $hotdate_main_images = ChotdatesTools::hotdate_images($hotdate_id);
                $url_files =$g['path']['url_files'];
                $photoMain = $hotdate_main_images['image_file'];
                $photoMainR = $hotdate_main_images['image_file'];
                $photoMainId = $hotdate_main_images['photo_id'];
            } elseif ($ehp_type == 'partyhou') {
                    $partyhou_id = self::getEHPId();
                    $partyhou = CpartyhouzTools::retrieve_partyhou_by_id($partyhou_id);

                    $infoName = array('name' => $partyhou['partyhou_title'], 'age' => '');
                    $nameShort = User::nameShort($partyhou['partyhou_title']);
                    $infoBlock = array('name' => self::getUserName($infoName, $partyhou_id),
                            'user_name' => $nameShort,
                            'user_name_attr' => toAttr($nameShort),
                            'user_name_title' => toAttr($partyhou['partyhou_title']),
                            'visible_name_short' => true,
                            'url' => "./partyhouz_partyhou_show.php?partyhou_id=" . $partyhou_id
                    );

                    $partyhou_main_images = CpartyhouzTools::partyhou_images($partyhou_id);
                    $url_files =$g['path']['url_files'];
                    $photoMain = $partyhou_main_images['image_file'];
                    $photoMainR = $partyhou_main_images['image_file'];
                    $photoMainId = $partyhou_main_images['photo_id'];
            }
        } else {
            if ($groupId) {
                ...
                $photoMain = $g['path']['url_files'] . User::getPhotoDefault($row['user_id'], $sizePhotoMain, false, '', DB_MAX_INDEX, false, false, false, false, $groupId);
                $photoMainR = $g['path']['url_files'] . User::getPhotoDefault($row['user_id'], 'r', false, '', DB_MAX_INDEX, false, false, false, false, $groupId);
                ...
                $photoMain = $g['path']['url_files'] . User::getPhotoDefault($row['user_id'], $sizePhotoMain, false, $row['gender']);
                $photoMainR = $g['path']['url_files'] . User::getPhotoDefault($row['user_id'], 'r', false, $row['gender']);
                $photoMainId = User::getPhotoDefault($row['user_id'], $sizePhotoMain, true);
            }
        }

        $infoBlock['photo'] = custom_getFileDirectUrl($photoMain);
        $infoBlock['photo_r'] = custom_getFileDirectUrl($photoMainR);        
        ...
        if ($groupId) {
            ...
            $module = 'member_ehp_additional_menu';
            $ehp_id = self::getEHPId();
            $ehp_type = self::getEHPType();
            if($ehp_type == 'event') {
                ...
                $user_id = $partyhou['user_id'];
            }

            if ($guid != $user_id) {
                $module = 'member_ehp_visited_additional_menu';
                if ($blockHeader != 'header_custom_big') {
                    $module = 'member_ehp_visited_additional_menu_inner';
                }
            }
        } else {
        ...
        if (!$isParseProfileCover) {
            ...
            if(self::isEHP()) {
                $profilePhoto = CProfilePhoto::preparePhotoListEHP();
                $profileVideo = array();
            }
            ...
            foreach ($profilePhoto as $id => $photo) {
                ...
                if ($isVideo) {
                    $urlPhoto = custom_getFileDirectUrl($g['path']['url_files'] . $photo['src_src']);
                } else {
                    $urlPhoto = custom_getFileDirectUrl($g['path']['url_files'] . $photo['src_b']);
                }
    ...
    static function parseColumnListImg(&$html, $uid, $type, $numColumn, $typeOrder, $blockColumnType, $groupId = 0)
    {
        ...
        foreach ($list as $id => $item) {
            ...
            if ($numColumn) {
                ...
                $urlPhoto = custom_getFileDirectUrl($g['path']['url_files'] . $item['src_s']);
    ...
    static function parseColumnListGroups(&$html, $uid, $type, $blockColumnType)
    {
        ...
        foreach ($list as $id => $item) {
            if ($numColumn) {
                ...
                $html->setvar("{$blockColumnItem}_photo_url", custom_getFileDirectUrl($g['path']['url_files'] . $urlPhoto));
            ...
            if ($type == 'subscribers_group' || $type == 'subscribers_group_online') {
                ...
                if ($friends) {
                    ...
                    foreach ($friends as $key => $friend) {
                        ...
                        $html->setvar("{$blockFriendItem}_name", $title);
                        $photo = custom_getFileDirectUrl($g['path']['url_files'] .  User::getPhotoDefault($friend['user_id'], 's', false, $friend['gender']));
                        $html->setvar("{$blockFriendItem}_photo", $photo);
                        $html->setvar("{$blockFriendItem}_url", User::url($friend['user_id']));
    ...
    static function parseListTabVideos(&$html, $typeOrder, $tab, $limit, $blockItems, $groupId = 0, $showAllMyVideo = false)
    {
        ...
        // $_GET['tags'] = '';
        $rows = CProfileVideo::getAccessVideosList($typeOrder, $tab, $limit, null, guid(), true, 0, '', $groupId, $showAllMyVideo);
        $numberRow = intval(self::getOptionList('list_videos_number_row'));
        $html->parse("list_video_{$postDisplayType}");
    ...
    static function parseVideo(&$html, $row, $numberRow, $postDisplayType)
    {
        ...
        if ($html->blockExists($blockItemType)) {
            ...
            $info = array('number_row'     => $numberRow,
                          ...
                          'subject'       => !empty($row['subject']) ? $row['subject'] : 'untitled',
                          'subject_attr'  => toAttr(!empty($row['subject']) ? $row['subject'] : 'untitled'),
                          'subject_short' => neat_trim(!empty($row['subject']) ? $row['subject'] : 'untitled', 100, ''),
                          'count_comments' => $row['count_comments'],
            ...
            /* Divyesh - Added on 11-04-2024 */
            if ($row['private'] === 'Y') {
                $html->setvar('private_title', l('remove_from_private'));
                $html->setvar("private", "remove_private");
                $html->setvar("private_icon", "minus-square");
            } else {
                $html->setvar('private_title', l('make_private'));
                $html->setvar("private_icon", "plus-square");
                $html->setvar("private", "private");
            }
            /* Divyesh - Added on 11-04-2024 */
            $html->assign($blockItem, $info);
            if (guid()) {
    ...
    static function parseUser(&$html, $row, $numberRow, $profileDisplayType, $blockItem = 'users_list_item')
    {
        ...
        if ($html->blockExists($blockItem)) {
            ...
            if (self::$listUserGroupId && self::$listUserGroupUid && self::$listUserGroupUid == $row['user_id']) {
                $isGroup = true;
                $photoUrl = custom_getFileDirectUrl($g['path']['url_files'] . GroupsPhoto::getPhotoDefault($row['user_id'], self::$listUserGroupId, 'bm'));
                ...
                if (Common::isOptionActive('location_enabled', 'edge_join_page_settings')) {
                    $infoProfile .= isset($row['city']) ? l($row['city']) : '';
                    $delimiter = ', ';
                }
                if (User::isShowAge($row)) {
            ...
            if( isset($row['group_moderator']) && $row['group_moderator'] == 'Y') {
                $html->parse("{$blockItem}_moderator_mark", false);
            } else {
                $html->clean("{$blockItem}_moderator_mark");
            }

            if(isset($row['group_user_id']) && $row['group_user_id'] == $row['user_id']) {
                $html->parse("{$blockItem}_owner_mark", false);
            } else {
                $html->clean("{$blockItem}_owner_mark");
            ...
            if ($p == 'groups_social_subscribers.php' && (isset($row['group_user_id']) && $row['group_user_id'] == guid() || isset($my_subscriber['group_moderator']) && $my_subscriber['group_moderator'] == 'Y')) {
            ...
            if (in_array($p, $pageParseMenu)) {
                ...
                if ($p == 'groups_social_block_list.php') {
                    ...
                    if (Common::isOptionActive('contact_blocking')) {
                        if((isset($row['group_user_id']) && $row['group_user_id'] == guid()) || $moderator_options['group_block']){
                    ...
                    if (!$row['page']) {
                        if ($row['group_user_id'] != $row['user_id']) {
                             ...
                             if ((isset($row['group_user_id']) && $row['group_user_id'] == guid()) || $moderator_options['group_leave']) {
                                $html->parse("{$blockItemMenu}_groups_unjoin", false);
                             }
                             if($row['user_id'] == guid() && $row['group_moderator'] == 'Y') {
                                $isParseMenu = false;
                            }
                    ...
                    if(Common::isOptionActive('group_moderator', 'edge_groups_settings')) {
                ...
                if($p == 'events_guest_users.php') {
                    ...
                    if($event_id) {
                        ...
                        if(isset($guest_user['user_id']) && $guest_user['accepted'] == 1) {
                            $html->parse("{$blockItem}_approved_mark", false);
                        } elseif(isset($guest_user['user_id']) && $guest_user['accepted'] == 0) {
                            $html->parse("{$blockItem}_pending_mark", false);
                        }
                        
                        $event_sql = "SELECT * FROM events_event WHERE event_id=" . to_sql($event_id);
                        $event = DB::row($event_sql);
                        if($event) {
                            if($event['user_id'] == guid()  && $row['user_id'] != $event['user_id']) {
                                $isParseMenu = true;
                                $html->setvar('guest_type', 'event');
                                $html->parse("{$blockItemMenu}_remove_guest", false);
                                $html->parse("{$blockItemMenu}_approve_guest", false);
                            }
                        }
                    }
                }
                elseif($p == 'hotdates_guest_users.php') {
                    ...
                    if($hotdate_id) {
                        ...
                        if($hotdate) {
                            if($hotdate['user_id'] == guid() && $row['user_id'] != $hotdate['user_id']) {
                                $isParseMenu = true;
                                $html->setvar('guest_type', 'hotdate');
                                $html->parse("{$blockItemMenu}_remove_guest", false);
                                $html->parse("{$blockItemMenu}_approve_guest", false);
                            }
                        }
                ...
                elseif($p == 'partyhouz_guest_users.php') {
                    ...
                    if($partyhou_id) {
                        ...
                        if($partyhou) {
                            if($partyhou['user_id'] == guid() && $partyhou['user_id'] != $row['user_id']) {
                                $isParseMenu = true;
                                $html->setvar('guest_type', 'partyhou');
                                $html->parse("{$blockItemMenu}_remove_guest", false);
                                $html->parse("{$blockItemMenu}_approve_guest", false);
                            }
                        }
            ...
            $html->clean("{$blockItem}_pending_mark");
        }
    }
            ...
            if ($layoutType == 'small') {
                ...
                foreach ($rows as $row) {
                    ...
                    $info = array('photo_id'  => $row['photo_id'],
                                  'src'       => custom_getFileDirectUrl($g['path']['url_files'] . $row['src_bm']),
    ...
    static function parseListPhotos(&$html, $typeOrder, $limit, $groupId = 0, $showAllMyPhoto = false, $user_id = 0)
    {
        $layoutType = 'default';
        if(self::isEHP()) {
            $rows = CProfilePhoto::preparePhotoListEHP($limit, $typeOrder, $user_id);
        } else {
    ...
    /* Divyesh - Added on 11-04-2024 */
    static function parseListTabPhotos(&$html, $typeOrder, $where, $limit, $groupId = 0, $showAllMyPhoto = false, $user_id = 0)
    {
        $layoutType = 'default';
        if(self::isEHP()) {
            $rows = CProfilePhoto::preparePhotoListEHP($limit, $typeOrder, $user_id);
        } else {
            $rows = CProfilePhoto::getAccessPhotosList($typeOrder, $where, $limit, null, $groupId, $showAllMyPhoto);
        }
    ...
    static function parseListTabsPhotos(&$html, $typeOrder, $where, $limit, $groupId = 0, $showAllMyPhoto = false, $user_id = 0)
    {
        $layoutType = 'default';
        
        $orderBy = CProfilePhoto::getOrderList($typeOrder);
        $rows = CProfilePhoto::prepareTabPhotoList(0, $orderBy, '', $limit, false, false, $where, $groupId, $showAllMyPhoto);
    ...
    static function getFriendsPending($where = '', $order = 'DESC',  $ajax = false)
    {
        ...
        foreach ($result as $key => $item) {
            ...
            $friendsPending[] = array(
                ...
                'url'      => $urlUserPending,
                'photo'    => custom_getFileDirectUrl($g['path']['url_files'] . User::getPhotoDefault($item['user_id'])),
