...
class CProfilePhoto extends CHtmlBlock
{
    ...
    public static function getTotalAccessedPhotos($uid = 0, $onlyPublic = false)
    {
        ...
        $sql = 'SELECT COUNT(DISTINCT PH.photo_id) FROM `photo` AS PH JOIN `user` AS U ON PH.user_id=U.user_id where ' . $where;
    ...
    public static function updateFaceDetectFriend($pid, $boxId, $uid)
    {
        ...
        $uid§³urrent = isset($faceDetectData['face'][$boxId]['uid']) ? $faceDetectData['face'][$boxId]['uid'] : 0;
        if (($uid && $photoInfo['user_id'] != $guid)
            || (!$uid && $uid§³urrent != $guid && $photoInfo['user_id'] != $guid)
        ) {
            return false;
        }
        ...
        if ($uid) {
            ...
            $where = '`photo_id` = ' . to_sql($pid) . ' AND `user_id` = ' . to_sql($uid§³urrent);
