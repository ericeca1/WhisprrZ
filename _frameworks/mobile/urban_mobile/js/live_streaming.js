var CLiveStreaming = function(guid) {
    ...
    this.like = function($el){
        ...
        $.post(url_ajax+'?cmd=live_stream_comment_like&view=mobile',
              {id:$this.lsVideoId,
               cid:cid,
               like:like},
            function(res){
    ...
    this.likeAdd = function($link, like) {
		...
		$.post(url_ajax+'?cmd=live_stream_like&view=mobile',{id:$this.lsVideoId, like:like}, function(data){
            if ($this.noAction()) return;
    ...
    this.loadMoreComments = function($el, limit){
        ...
        var fnLoad=function(){
            ...
            $.ajax({url:url_ajax+'?view=mobile&cmd='+cmd,
                    type:'POST',
                    data:dataRes,
    ...
    this.commentsRepliesLoadMore = function($el, cid, limit) {
        ...
        var fnLoad=function(){
            $.ajax({url:url_ajax+'?cmd=get_live_stream_comment_replies&view=mobile',
                type:'POST',
                data:dataRes,
                timeout: globalTimeoutAjax,
                //cache: false,
    ...
    this.commentAdd = function(inp) {
        ...
        var fnAdd=function(){
            ...
            var data={comment:comment,
                      ...
                      live_id:$this.lsId,
                      reply_id:rCid};
            $.post(url_ajax+'?cmd=live_stream_comment_add&view=mobile',data,
            function(res){
                if ($this.noAction()) return;
    ...
    this.confirmDeleteComment = function($el,cid,rCid){
		showConfirm(l('confirm_delete_comment'), l('ok'), l('cancel'), function(){$this.commentDelete($el,cid,rCid)}, 'red', '#st-container');
    }
    ...
    this.commentDelete = function($el,cid,rCid) {
        ...
        $.post(url_ajax+'?cmd=live_stream_comment_delete&view=mobile',data,
                function(res){
                    if($this.noAction())return;
                    var data=checkDataAjax(res);
                    if (data !== false){
...
/* ----------------------- CACHE DATA ----------------------------------- */
    ...
    this.searchLive = function($el, prev){
        ...
        //liveId = 9;
        /* Test */
        $.post(url_ajax+'?cmd=live_search&view=mobile',
              {cur_live_id:liveId,
               prev:prev},
            function(res){
                var data=checkDataAjax(res);
                if (data !== false){
                    if (data) {
                        ...
                        $bl.removeClass('to_show');
						showAlert(l('no_more_live_broadcasts_currently'),'#st-container','blue');
                    }
                    /* Test */
                    //$bl.removeClass('disabled');
                    //$el.removeChildrenLoader();
	...
	this.reportAjax=false;
	this.reportPp=[];
	this.reportLive = function(data){
		$this.reportData=data;
		if(!$this.reportPp[0]){
			$this.reportPp=$('<div id="pp_report_user" class="popup pp_cont">'+
                '<div class="cont_pp"><div class="field">'+
					'<textarea id="pp_report_user_text" maxlength="255" type="text" class="placeholder" placeholder="'+l('report_this_content_to_administrator')+'"></textarea>'+
                    '</div>'+
					'<button onclick="clStream.reportSend(); return false;" type="button" data-no-fade-in="true" class="btn large violet">'+
					'<span>'+l('add_a_message_for_the_administrator')+'</span>'+
					'</button>'+
				'</div></div>').modalPopup({css:{}, wrClass:'pp_wrapper_live', shClass:'pp_shadow_live'});

			$('body').click(function(e){
				$targ=$(e.target);
				if (!$targ.is('#pp_report_user')&&!$targ.closest('#pp_report_user')[0]
					&&!$targ.is('.feed_btn_report')&&!$targ.closest('.feed_btn_report')[0]) {
					$this.reportPp.close(350)
				}
				hideAlert();
			})
		}
		$('#pp_report_user_text').val('');
		$this.reportPp.open(350,function(){
			//$('#pp_report_user_text').focus();
		})
	}

	this.reportSend = function(){
		var msg=trim($('#pp_report_user_text').val());
		if($this.reportAjax||!msg)return false;
		$this.reportAjax=true;

		$.post(url_ajax+'?cmd=report_user',
              {user_to:$this.reportData.uid, msg:msg, photo_id:$this.reportData.lid},
                function(res){
					if(checkDataAjax(res)){
						$this.reportPp.close(350,function(){
							showAlert(l('report_sent'),'body','blue');
